...
const Checkout = {
    ...
    renderSummary: function() {
        ...
        const itemPriceEl = document.getElementById('ml-sum-item-price');
        ...
        // Atualizar o preco unitario
        if (itemPriceEl && state.items && state.items.length > 0) {
            itemPriceEl.textContent = this.formatCurrency(state.items[0].unit_price);
        }
        
        // Atualizar o span de quantidade
        if (qtySpan && state.items && state.items.length > 0) {
            qtySpan.textContent = state.items[0].quantity || 1;
        }
    ...
    // ─── QUANTIDADE ──────────────────────────────────────────────────────────
    changeQty: function(itemIndex, change) {
        const state = this.getState();
        if (!state.items || !state.items[itemIndex]) return;
        
        let newQty = (state.items[itemIndex].quantity || 1) + change;
        newQty = Math.max(1, Math.min(newQty, 100));
        
        state.items[itemIndex].quantity = newQty;
        state.amount = state.items[itemIndex].unit_price * newQty;
    ...
    updateQuantityFromInput: function() {
        const input = document.getElementById('ml-qty-input');
        if (!input) return;
        ...
        // Atualizar o amount baseado na quantidade
        if (state.items && state.items.length > 0) {
            const baseUnitPrice = state.items[0].unit_price;
            state.items[0].quantity = quantity;
            state.amount = baseUnitPrice * quantity;
        }
